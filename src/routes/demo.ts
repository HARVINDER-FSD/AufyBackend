...
router.post("/seed", async (req, res) => {
  try {
    ...
    let demoUser = await usersCollection.findOne({ username: 'demo_user' })
    ...
    const reelsResult = await reelsCollection.insertMany(reels)
...
router.delete("/clear", async (req, res) => {
  try {
    ...
    // Find demo user
    const demoUser = await usersCollection.findOne({ username: 'demo_user' })
    ...
    const postsDeleted = await postsCollection.deleteMany({ user_id: demoUser._id })
    const reelsDeleted = await reelsCollection.deleteMany({ user_id: demoUser._id })
...
router.get("/stats", async (req, res) => {
  try {
    ...
    const postsCount = await postsCollection.countDocuments({ user_id: demoUser._id })
    const reelsCount = await reelsCollection.countDocuments({ user_id: demoUser._id })
