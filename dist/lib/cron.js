...
class CronScheduler {
    ...
    schedule(name, taskType, data, intervalMs) {
    ...
    start() {
        ...
        this.schedule('cleanup_expired_stories', 'cleanup_expired_data', { type: 'stories', olderThan: new Date() }, 60 * 60 * 1000 // 1 hour
        );
        // Clean up expired sessions every 30 minutes
        this.schedule('cleanup_expired_sessions', 'cleanup_expired_data', { type: 'sessions', olderThan: new Date() }, 30 * 60 * 1000 // 30 minutes
        );
        // Clean up temporary files every 2 hours
        this.schedule('cleanup_temp_files', 'cleanup_expired_data', { type: 'temp_files', olderThan: new Date(Date.now() - 2 * 60 * 60 * 1000) }, 2 * 60 * 60 * 1000 // 2 hours
        );
        // Generate daily analytics reports at midnight
        this.schedule('daily_analytics', 'generate_analytics_report', { period: 'daily' }, 24 * 60 * 60 * 1000 // 24 hours
        );
        // Weekly user data backup on Sundays at 2 AM
        this.schedule('weekly_backup', 'backup_user_data', { backupType: 'weekly' }, 7 * 24 * 60 * 60 * 1000 // 7 days
        );
