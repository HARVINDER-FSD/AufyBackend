...
async function connectToMongoDB() {
    try {
        if (mongoose_1.default.connection.readyState === 0) {
            await mongoose_1.default.connect(process.env.MONGODB_URI || 'mongodb://127.0.0.1:27017/socialmedia');
            console.log('Connected to MongoDB');
            // Check if we have any users, if not create a default one
            const userCount = await types_1.User.countDocuments();
            if (userCount === 0) {
                const hashedPassword = bcrypt.hashSync("password123", 10);
                await types_1.User.create({
                    username: "testuser",
...
async function verifyAuth(req) {
    try {
        ...
        try {
            if (database_1.redis) {
                const exists = await database_1.redis.exists(`session:${decoded.userId}:${accessToken}`);
                if (!exists) {
...
class AuthService {
    ...
    static async register(userData) {
        ...
        try {
            ...
            const existingUser = users.find(u => u.username.toLowerCase() === sanitizedUsername.toLowerCase() ||
    ...
    static async login(credentials, deviceInfo) {
        ...
        try {
            if (database_1.redis) {
                await database_1.cache.set(`user:${user.id}`, userData, config_1.config.redis.ttl.user);
            }
        }
        catch (error) {
    ...
    // Create session
    static async createSession(userId, accessToken, deviceInfo) {
        // Store in Redis for quick access if available
        if (database_1.redis) {
            try {
                await database_1.redis.set(`session:${userId}:${accessToken}`, JSON.stringify({
                    userId,
                    deviceInfo,
