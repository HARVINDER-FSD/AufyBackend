...
async function getDb() {
    ...
    cachedClient = await mongodb_1.MongoClient.connect(MONGODB_URI, {
        maxPoolSize: 10,
        minPoolSize: 2
    });
...
router.get('/me', authenticate, async (req, res) => {
    try {
        ...
        const user = await db.collection('users').findOne({ _id: new mongodb_1.ObjectId(userId) }, { projection: { password: 0 } } // Don't fetch password
        );
        if (!user) {
...
router.put('/profile', authenticate, async (req, res) => {
    try {
        ...
        let idQuery;
        try {
            user = await db.collection('users').findOne({ _id: new mongodb_1.ObjectId(userId) });
        ...
        try {
            const result = await db.collection('users').updateOne(idQuery, { $set: updateData });
...
router.post('/:userId/follow', authenticate, async (req, res) => {
    try {
        ...
        const existingFollow = await db.collection('follows').findOne({
            followerId: new mongodb_1.ObjectId(currentUserId),
            followingId: new mongodb_1.ObjectId(userId)
        });
        if (existingFollow) {
            // Unfollow
...
router.delete('/:userId/follow', authenticate, async (req, res) => {
    try {
        ...
        // Update counts
        await db.collection('users').updateOne({ _id: new mongodb_1.ObjectId(currentUserId) }, { $inc: { following: -1 } });
        await db.collection('users').updateOne({ _id: new mongodb_1.ObjectId(userId) }, { $inc: { followers: -1 } });
        await client.close();
...
router.get('/:userId/followers/debug', async (req, res) => {
    try {
        const { userId } = req.params;
        const client = await mongodb_1.MongoClient.connect(MONGODB_URI);
        const db = client.db();
...
router.delete('/delete', authenticate, async (req, res) => {
    try {
        ...
        const db = client.db();
        // Delete user
        await db.collection('users').deleteOne({ _id: new mongodb_1.ObjectId(userId) });
        // Delete user's posts
        await db.collection('posts').deleteMany({ userId: new mongodb_1.ObjectId(userId) });
