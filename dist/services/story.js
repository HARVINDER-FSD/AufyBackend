...
class StoryService {
    ...
    static async createStory(userId, storyData) {
        ...
        const result = await (0, database_1.query)(`INSERT INTO stories (user_id, media_url, media_type, content) 
       VALUES ($1, $2, $3, $4) 
       RETURNING id, user_id, media_url, media_type, content, expires_at, is_archived, created_at`, [userId, media_url, media_type, content]);
        ...
        await database_1.cache.del(utils_1.cacheKeys.userStories(userId));
        await database_1.cache.invalidatePattern(`${config_1.config.redis.keyPrefix}stories:*`);
        return storyWithUser;
    }
    // Get user stories
    static async getUserStories(userId, currentUserId) {
        // Check cache first
        const cacheKey = utils_1.cacheKeys.userStories(userId);
        const cachedStories = await database_1.cache.get(cacheKey);
        if (cachedStories && !currentUserId) {
            return cachedStories;
        }
        const result = await (0, database_1.query)(`SELECT s.id, s.user_id, s.media_url, s.media_type, s.content, 
       ...
       ORDER BY s.created_at DESC`, [userId]);
        const stories = await Promise.all(result.rows.map(async (row) => {
            ...
            if (currentUserId) {
                const viewResult = await (0, database_1.query)("SELECT id FROM story_views WHERE story_id = $1 AND viewer_id = $2", [
                    story.id,
                    currentUserId,
                ]);
        ...
        if (!currentUserId) {
            await database_1.cache.set(cacheKey, stories, config_1.config.redis.ttl.story);
        }
        return stories;
    }
    // Get stories feed (stories from followed users)
    static async getStoriesFeed(userId) {
        const result = await (0, database_1.query)(`SELECT DISTINCT s.user_id,
    ...
    static async viewStory(storyId, viewerId) {
        ...
        const storyResult = await (0, database_1.query)("SELECT id, user_id FROM stories WHERE id = $1 AND expires_at > NOW() AND is_archived = false", [storyId]);
        ...
        await (0, database_1.query)(`INSERT INTO story_views (story_id, viewer_id) 
       VALUES ($1, $2) 
       ON CONFLICT (story_id, viewer_id) DO NOTHING`, [storyId, viewerId]);
        // Clear cache
        await database_1.cache.del(utils_1.cacheKeys.userStories(story.user_id));
    }
    // Get story views
    static async getStoryViews(storyId, userId) {
        // Verify story ownership
        const storyResult = await (0, database_1.query)("SELECT user_id FROM stories WHERE id = $1", [storyId]);
    ...
    static async deleteStory(storyId, userId) {
        ...
        const storyCheck = await (0, database_1.query)("SELECT id, user_id, media_url FROM stories WHERE id = $1", [storyId]);
        ...
        try {
            ...
            await storage_1.StorageService.deleteFile(key);
        }
        catch (error) {
    ...
    static async archiveExpiredStories() {
        const result = await (0, database_1.query)("UPDATE stories SET is_archived = true WHERE expires_at <= NOW() AND is_archived = false");
