...
class ReelService {
    // Create reel
    static async createReel(userId, reelData) {
    ...
    // Get reel by ID
    static async getReelById(reelId, currentUserId) {
        const db = await (0, database_1.getDatabase)();
        const reelsCollection = db.collection('reels');
        const usersCollection = db.collection('users');
    ...
    // Get user's reels
    static async getUserReels(userId, currentUserId, page = 1, limit = 20) {
    ...
    static async getReelsFeed(currentUserId, page = 1, limit = 20) {
        const { page: validPage, limit: validLimit } = utils_1.pagination.validateParams(page.toString(), limit.toString());
        const offset = utils_1.pagination.getOffset(validPage, validLimit);
    ...
    static async deleteReel(reelId, userId) {
        ...
        // Soft delete
        await reelsCollection.updateOne({ _id: new mongodb_1.ObjectId(reelId) }, {
            $set: {
                is_deleted: true,
                updated_at: new Date()
            }
        ...
        // Clear cache
        await database_1.cache.del(`reel:${reelId}`);
    }
    // Like reel
    static async likeReel(userId, reelId) {
        const db = await (0, database_1.getDatabase)();
        const reelsCollection = db.collection('reels');
        const likesCollection = db.collection('likes');
        const reel = await reelsCollection.findOne({
    ...
    // Unlike reel
    static async unlikeReel(userId, reelId) {
        const db = await (0, database_1.getDatabase)();
        const likesCollection = db.collection('likes');
    ...
    // Get reel likes
    static async getReelLikes(reelId, page = 1, limit = 20) {
        const { page: validPage, limit: validLimit } = utils_1.pagination.validateParams(page.toString(), limit.toString());
        const offset = utils_1.pagination.getOffset(validPage, validLimit);
        const db = await (0, database_1.getDatabase)();
        const likesCollection = db.collection('likes');
        const total = await likesCollection.countDocuments({ post_id: new mongodb_1.ObjectId(reelId) });
